...
{
	dacDevice->i2cAddress = 0x20;
	HAL_I2C_Master_Transmit(dacDevice->i2cHandler, dacDevice->i2cAddress, dacDevice->txBuffer, 3, 100);
}

void MAX5814_ReadCommand(MAX5814_Handler_t *dacDevice)
{
	dacDevice->i2cAddress = 0x20 | MAX5814_ADDRESS;
	HAL_I2C_Master_Transmit(dacDevice->i2cHandler, dacDevice->i2cAddress, dacDevice->txBuffer, 3, 100);
}
